...
# Check for duplicate coordinates and aggregate them
cat("\nChecking for duplicate coordinates...\n")

# Create initial coordinate data
...
# Find and report duplicate coordinates before aggregation
dup_coords_check <- coord_data %>%
...
if(nrow(dup_coords_check) > 0) {
  cat("Found", nrow(dup_coords_check), "coordinate pairs with multiple records\n")
  cat("These will be aggregated by summing positive cases and sample sizes\n")
  
  # Show first few examples
  for(i in seq_len(min(5, nrow(dup_coords_check)))) {
    coord <- dup_coords_check[i,]
    matching_rows <- coord_data[coord_data$longitude == coord$longitude & 
                               coord_data$latitude == coord$latitude, ]
    
    cat("\nCoordinate (", coord$longitude, ",", coord$latitude, "):\n")
    cat("  Records:", coord$n_records, "\n")
    cat("  Before aggregation:\n")
    for(j in seq_len(nrow(matching_rows))) {
      cat("    Record", j, ": positive =", matching_rows$positive[j], 
          ", sample_size =", matching_rows$sample_size[j], 
          ", prevalence =", round(matching_rows$positive[j]/matching_rows$sample_size[j], 3), "\n")
    }
    cat("  After aggregation: positive =", sum(matching_rows$positive), 
        ", sample_size =", sum(matching_rows$sample_size),
        ", prevalence =", round(sum(matching_rows$positive)/sum(matching_rows$sample_size), 3), "\n")
  }
  
  if(nrow(dup_coords_check) > 5) {
    cat("... and", nrow(dup_coords_check) - 5, "more duplicate coordinate pairs\n")
  }
} else {
  cat("No duplicate coordinates found - all locations are unique\n")
}

# Aggregate duplicate coordinates by summing positive cases and sample sizes
aggregated_data <- coord_data %>%
  group_by(longitude, latitude) %>%
  summarise(
    positive = sum(positive),
    sample_size = sum(sample_size),
    n_records_combined = n(),
    .groups = 'drop'
  )

cat("\nData aggregation summary:\n")
cat("- Original records:", nrow(coord_data), "\n")
cat("- Unique locations after aggregation:", nrow(aggregated_data), "\n")
cat("- Records combined:", nrow(coord_data) - nrow(aggregated_data), "\n")

# Update the main data vectors and data object with aggregated values
positive <- aggregated_data$positive
sample_size <- aggregated_data$sample_size

# Update the data object to match the aggregated data
# We need to create a new data frame with aggregated coordinates
data_aggregated <- data.frame(
  Longitude = aggregated_data$longitude,
  Latitude = aggregated_data$latitude
)

# Replace the original data with aggregated version for coordinate extraction
data <- data_aggregated

cat("- New data length:", length(positive), "\n")
...
mesh <- inla.mesh.2d(
  ...
  max.edge = c(6, 15)        # Increased from c(30, 60) - much more reasonable
)
