...
if(countries_to_infer == "Ethiopia") {
  neighboring_countries <- c("BEN", "NER", "TCD", "CMR")  # Benin, Niger, Chad, Cameroon
} else if(countries_to_infer == "Ethiopia") {
...
if(countries_to_infer == "Ethiopia") {
  # Get Ethiopian administrative boundaries at district level (level 2)
  admin_level <- 3
  admin_name_col <- "NAME_2"
  tryCatch({
    ethiopia_admin <- gadm(country = "ETH", level = admin_level, path = tempdir())
    admin_sf <- st_as_sf(ethiopia_admin)
    admin_sf <- st_make_valid(admin_sf)
    cat("Successfully loaded Ethiopia district boundaries\n")
  }, error = function(e) {
    cat("Failed to load Ethiopia district boundaries:", e$message, "\n")
  })
}
...
aggregate_by_admin <- function(projections_data, variable_name) {
  ...
  admin_prevalence <- joined_data %>%
    st_drop_geometry() %>%
    dplyr::filter(!is.na(GID_3)) %>%  # Only keep points within admin units
    dplyr::group_by(GID_3, !!sym(admin_name_col), NAME_1) %>%
    dplyr::summarise(
  ...
  all_admin <- admin_sf %>%
    st_drop_geometry() %>%
    dplyr::select(GID_3, !!sym(admin_name_col), NAME_1)
  
  missing_admin <- all_admin %>%
    dplyr::anti_join(admin_prevalence, by = "GID_3")
...
admin_uncertainty <- admin_prevalence_lower %>%
  dplyr::select(GID_3, lower = admin_prevalence) %>%
  left_join(admin_prevalence_upper %>% dplyr::select(GID_3, upper = admin_prevalence), by = "GID_3") %>%
  dplyr::mutate(
...
create_enhanced_map <- function(admin_data, title_suffix, show_uncertainty = FALSE) {
  ...
  admin_final <- admin_sf %>%
    left_join(admin_data, by = "GID_3")
  
  # Add uncertainty information if requested
  if(show_uncertainty) {
    admin_final <- admin_final %>%
      left_join(admin_uncertainty, by = "GID_3")
  }
  ...
  p <- p +
    ...
    labs(
      title = paste("AAT prevalence by", 
                    ifelse(countries_to_infer == "Ethiopia", "district", "LGA"), 
...
create_histogram_plot <- function(admin_data, estimate_name) {
  ...
  admin_unit <- ifelse(countries_to_infer == "Ethiopia", "district", "LGA")
...
#write.csv(water_cover, "Code/Prevalence/Bovine BCT and PCR/Analysis_ETH/ETH_water_cover.csv", row.names = FALSE)
