...
# Get Ethiopian administrative boundaries
cat("Loading Ethiopian administrative boundaries...\n")
ethiopia_states <- gadm(country = "ETH", level = 1, path = tempdir()) 
ethiopia_states_sf <- st_as_sf(ethiopia_states)

# Get neighboring countries for geographic context
cat("Loading neighboring countries for geographic context...\n")
neighboring_countries <- c("ERI", "SOM", "KEN", "SSD", "SDN", "DJI","UGA")  # Eritrea, Somalia, Kenya, South Sudan, Sudan, Djibouti
...
if(length(neighbor_sf_list) > 0) {
  ...
  ethiopia_states_valid <- st_make_valid(ethiopia_states_sf)
  
  # Create a buffer around Ethiopia to crop neighboring countries to border regions only
  ethiopia_buffer <- st_buffer(st_union(ethiopia_states_valid), dist = 1.5)  # ~150km buffer
  ethiopia_buffer <- st_make_valid(ethiopia_buffer)
  
  # Use st_crop instead of st_intersection for more robust clipping
  tryCatch({
    neighboring_countries <- st_crop(neighboring_countries_full, ethiopia_buffer)
    ...
    neighboring_countries <<- st_crop(neighboring_countries_full, ethiopia_bbox)
...
bovine_bct_raw <- readxl::read_excel("Data/ContAtlas_v2/Bovine data/AAT_PR_bovine_BCT-HCT_data_table.xls")
bovine_pcr_raw <- readxl::read_excel("Data/ContAtlas_v2/Bovine data/AT_PREV_bovine_PCR_Table.xls")
...
cat("Processing bovine data for Ethiopia...\n")
...
bovine_ethiopia <- st_filter(bovine_sf, ethiopia_states_sf)
...
cat("Processed", nrow(bovine_ethiopia), "observations in Ethiopia\n")
cat("Test types available:", unique(bovine_ethiopia$Test_Type), "\n")
cat("Sample size distribution:\n")
print(table(bovine_ethiopia$Sample_Size_Category, bovine_ethiopia$Test_Type))

# Calculate maximum prevalence for scale capping
max_prevalence <- max(bovine_ethiopia$Prevalence, na.rm = TRUE)
...
# Get Ethiopia bounds for plot limits
ethiopia_bbox <- st_bbox(ethiopia_states_sf)
lon_range <- ethiopia_bbox[c("xmin", "xmax")]
lat_range <- ethiopia_bbox[c("ymin", "ymax")]
...
p <- ggplot() +
  ...
  # Add Ethiopia state boundaries with single clean border
  geom_sf(data = ethiopia_states_sf, fill = "white", color = "gray50", size = 0.3) +
  
  # Add bovine data points
  geom_point(data = as.data.frame(bovine_ethiopia), 
             aes(x = lon, y = lat, 
                 color = Prevalence,
  ...
  theme_void() +
  labs(
    title = "Raw AAT survey data - Ethiopia",
    subtitle = paste0("N = ", nrow(bovine_ethiopia), " observations | ", 
                     "BCT/HCT: ", sum(bovine_ethiopia$Test_Type == "BCT/HCT"),
                     " | PCR: ", sum(bovine_ethiopia$Test_Type == "PCR")),
...
# Save the map plot
output_file <- "Code/Prevalence/Bovine BCT and PCR/Analysis_ETH/Raw_AAT_Data_Ethiopia.png"
...
h <- ggplot(hist_data, aes(x = Prevalence * 100)) +
  ...
  labs(
    title = "Distribution of AAT Prevalence - Ethiopia",
    subtitle = paste0("Based on ", nrow(hist_data), " observations | Max prevalence: ", 
                     round(max_prevalence * 100, 1), "%")
  )

print(h)

# Save histogram
hist_file <- "Code/Prevalence/Bovine BCT and PCR/Analysis_ETH/AAT_Prevalence_Histogram_Ethiopia.png"
ggsave(hist_file, plot = h, width = 10, height = 8, dpi = 300, bg = "white")
cat("Histogram saved to:", hist_file, "\n")

# Print summary statistics
cat("\n=== SUMMARY STATISTICS ===\n")
cat("Total observations in Ethiopia:", nrow(bovine_ethiopia), "\n")
cat("By test type:\n")
test_summary <- bovine_ethiopia %>%
  st_drop_geometry() %>%
  group_by(Test_Type) %>%
  summarise(
    n_obs = n(),
