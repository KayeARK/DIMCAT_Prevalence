...
bovine_bct_raw <- read_excel("Data/ContAtlas_v2/Bovine data/AAT_PR_bovine_BCT-HCT_data_table.xls")
bovine_pcr_raw <- read_excel("Data/ContAtlas_v2/Bovine data/AT_PREV_bovine_PCR_Table.xls")

# Add test type identifier to each dataset
bovine_bct_raw$Test_Type <- "BCT"
bovine_pcr_raw$Test_Type <- "PCR"

# Standardize column names - rename prevalence columns to match
...
bovine_data_raw <- rbind(bovine_bct_raw, bovine_pcr_raw)
...
create_uncertainty_combined_plot <- function(lga_data, estimate_name, x_label) {
  ...
  p_hist <- ggplot(lga_data, aes(x = lga_value, fill = after_stat(x))) +
    ...
    scale_x_continuous(labels = if(max(lga_data$lga_value, na.rm = TRUE) <= 1) scales::percent_format() else scales::number_format(accuracy = 0.001),
                       ...
                       limits = c(min(lga_data$lga_value, na.rm = TRUE), max(lga_data$lga_value, na.rm = TRUE)),
    ...
    labs(
      title = paste("Distribution of", estimate_name, "by LGA"),
      x = ""
    )
...
p_rel_between <- create_uncertainty_choropleth(lga_rel_between, "(relative variability from data uncertainty)", "Relative\nimportance", c(0, 1))
p_rel_within <- create_uncertainty_choropleth(lga_rel_within, "(relative variability from model uncertainty)", "Relative\nimportance", c(0, 1))
p_between_var <- create_uncertainty_choropleth(lga_between_var, "(Variance from data uncertainty)", "Variance", c(min(lga_between_var$lga_value, na.rm = TRUE), max(lga_between_var$lga_value, na.rm = TRUE)))

# Create combined histogram and boxplot analyses
p_rel_between_combined <- create_uncertainty_combined_plot(lga_rel_between, "relative variability from data uncertainty", "Relative importance of variability from data uncertainty")
p_rel_within_combined <- create_uncertainty_combined_plot(lga_rel_within, "relative variability from model uncertainty", "Relative importance of variability from model uncertainty")
p_between_var_combined <- create_uncertainty_combined_plot(lga_between_var, "variance from data uncertainty", "Variance from data uncertainty")

# Save all choropleth maps
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/lga_rel_between_choropleth.pdf", plot = p_rel_between, width = 12, height = 10)
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/lga_rel_within_choropleth.pdf", plot = p_rel_within, width = 12, height = 10)
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/lga_between_var_choropleth.pdf", plot = p_between_var, width = 12, height = 10)

# Save combined plots (histogram + boxplot for each uncertainty measure)
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/lga_rel_between_analysis.pdf", plot = p_rel_between_combined, width = 10, height = 6)
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/lga_rel_within_analysis.pdf", plot = p_rel_within_combined, width = 10, height = 6)
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/lga_between_var_analysis.pdf", plot = p_between_var_combined, width = 10, height = 6)
