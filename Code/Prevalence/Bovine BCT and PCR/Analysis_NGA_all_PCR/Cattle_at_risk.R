...
i<-1
data <- read.csv(paste0("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/Cattle_at_risk/Projections_model_",i,".csv"))

locations <-  data[,1]

#set up an empty array of size 34*200
cattle_at_risk <- array(NA, dim = c(length(locations),200))
prevalence <- array(NA, dim = c(length(locations),200))
...
colnames(prevalence_tibble) <- paste0("Sim_", 1:200)
...
ggsave("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/Prevalence_plot.pdf", width=8, height=6)
...
ggsave(paste0("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/Cattle_at_risk_plots/Histogram_location_",locations[i],".png"), plot=p)
}
...
for (i in 1:38){
  ...
  ggsave(paste0("Code/Prevalence/Bovine BCT and PCR/Analysis_NGA_all_PCR/Cattle_at_risk_plots/Histogram_prevalence_location_",locations[i],".png"), plot=p)
}
