...
projections_dir <- "Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Projections_ETH_mean/"
...
aggregate_by_zone <- function(projections_sf) {
  ...
  missing_zones <- all_zones %>%
    dplyr::anti_join(zone_prevalence, by = "GID_2")
...
zone_uncertainty <- zone_prevalence_lower %>%
  dplyr::select(GID_2, lower = lower_prevalence) %>%
  left_join(zone_prevalence_upper %>% dplyr::select(GID_2, upper = upper_prevalence), by = "GID_2") %>%
  dplyr::mutate(
...
create_choropleth <- function(lga_data, title_suffix, data_type = "mean", add_bovine = FALSE, show_uncertainty = FALSE, add_inset = FALSE) {
  ...
  ethiopia_zones_final <- ethiopia_zones_sf %>%
    left_join(lga_data %>% st_drop_geometry() %>% dplyr::select(GID_2, all_of(fill_column)), 
              by = "GID_2")
  
  # Add uncertainty information if requested
  if(show_uncertainty) {
    ethiopia_zones_final <- ethiopia_zones_final %>%
      left_join(zone_uncertainty, by = "GID_2")
  }
  ...
  if(add_inset && !is.null(africa_countries_sf) && !is.null(ethiopia_country_sf)) {
    ...
    p <- ggdraw(p) +
      draw_plot(inset_plot, x = 0.70, y = 0.70, width = 0.25, height = 0.25)
  }
...
p_mean <- create_choropleth(zone_prevalence_mean, "(mean)", show_uncertainty = FALSE, add_inset = TRUE)
...
create_combined_plot <- function(lga_data, estimate_name) {
  ...
  p_hist <- ggplot(plot_data, aes(x = mean_prevalence, fill = after_stat(x))) +
    ...
    labs(
      title = paste("Distribution of", estimate_name, "prevalence by zone"),
      x = ""
    )
...
# Save all plots to Analysis_ETH_mean directory
ggsave("Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Analysis_ETH_mean/eth_choropleth_mean.pdf", plot = p_mean, width = 12, height = 10)
ggsave("Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Analysis_ETH_mean/eth_choropleth_lower.pdf", plot = p_lower, width = 12, height = 10)
ggsave("Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Analysis_ETH_mean/eth_choropleth_upper.pdf", plot = p_upper, width = 12, height = 10)

# Save combined plots (histogram + boxplot for each estimate type) - more compact dimensions
ggsave("Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Analysis_ETH_mean/eth_mean_analysis.pdf", plot = p_mean_combined, width = 10, height = 6)
ggsave("Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Analysis_ETH_mean/eth_lower_analysis.pdf", plot = p_lower_combined, width = 10, height = 6)
ggsave("Code/Prevalence/Bovine BCT and PCR/Harriet_Layer/Analysis_ETH_mean/eth_upper_analysis.pdf", plot = p_upper_combined, width = 10, height = 6)
