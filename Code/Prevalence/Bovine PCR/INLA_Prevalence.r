...
covariates <- read.csv(paste0("Code/Prevalence/Bovine BCT/Results/",countries_to_infer,"Covariates.csv"))
#change to array
...
ggsave(paste0("Code/Prevalence/Bovine BCT/Results/",countries_to_infer,"Predictions.png"),width = 12, height = 8)
...
#plot prevalence against mode in a scatter plot with a line y=x using ggplot
prevalence <- prevalence[!is.na(prevalence)]
modes <- modes[!is.na(modes)]
dpm <- dpm[!is.na(dpm$value),]
dpm <- data.frame(Latitude = dpm$Latitude, Longitude = dpm$Longitude, value = dpm$value, variable = dpm$variable)
ggplot(data.frame(prevalence, modes)) +
  geom_point(aes(x = prevalence, y = modes)) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(x = "Prevalence", y = "Mode") +
  theme_bw()
#save
ggsave(paste0("Code/Prevalence/Bovine BCT/Results/",countries_to_infer,"Prevalence_vs_Mode.png"),width = 8, height = 6)

print(cor(prevalence,modes))

#Latitude_means <-dpm[1:18304,1,]
#Longitude_means <-dpm[1:18304,2,]
#means <- dpm[1:18304,3,]
