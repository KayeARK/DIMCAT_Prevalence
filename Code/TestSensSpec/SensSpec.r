...
model {
  ...
  mu_logit_Se_pcr ~ normal(logit(0.5), 1);  // centers Se_pcr ~ 0.5
  mu_logit_Sp_pcr ~ normal(logit(0.5), 1);  // centers Sp_pcr ~ 0.5
  mu_logit_Se_hct ~ normal(logit(0.5), 1); // centers Se_hct ~ 0.5
  mu_logit_Sp_hct ~ normal(logit(0.5), 1); // centers Sp_hct ~ 0.5
...
# Density plots (Se)
p1 <- ggplot(reshape2::melt(df_plot[,c("Se_pcr","Se_hct")]), aes(x=value, color=variable)) +
  geom_density() + labs(title="Posterior densities: sensitivity", x="Sensitivity", y="Density") +
  theme_minimal()
print(p1)
#save as pdf
ggsave("Code/TestSensSpec/sensitivity_density.pdf", plot = p1, width = 8, height = 6)

p2 <- ggplot(reshape2::melt(df_plot[,c("Sp_pcr","Sp_hct")]), aes(x=value, color=variable)) +
  geom_density() + labs(title="Posterior densities: specificity", x="Specificity", y="Density") +
  theme_minimal()
print(p2)
#save as pdf
ggsave("Code/TestSensSpec/specificity_density.pdf", plot = p2, width = 8, height = 6)

p3 <- ggplot(reshape2::melt(df_plot[,c("rel_Se","rel_Sp")]), aes(x=value, color=variable)) +
  geom_density() + labs(title="Posterior densities: relative metrics (HCT / PCR)", x="Ratio", y="Density") +
  theme_minimal()
print(p3)
#save as pdf
ggsave("Code/TestSensSpec/relative_metrics_density.pdf", plot = p3, width = 8, height = 6)


# Save fit for later
saveRDS(fit, file = "Code/TestSensSpec/latent_class_fit.rds")
message("Fit saved to latent_class_fit.rds")

#assess model convergence
mcmc_trace(fit, pars = c("Se_pcr", "Sp_pcr", "Se_hct", "Sp_hct"))
