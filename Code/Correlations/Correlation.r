...
data$tsetse <-terra::extract(r_tsetse, data[, c("Longitude", "Latitude")])
...
#   geom_point() +
#   labs(title = paste("Correlation between Elevation and Precipitation in Nigeria: ", round(correlation, 2)),
#        x = "Elevation (m)",
#        y = "Precipitation (mm)")
# ggsave("Code/AbsencePresence/Correlation.png", width = 8, height = 6)

#compute correlation between all variables
correlation_matrix <- cor(data[, c("elevation", "precipitation", "tavg", "tmin", "tmax", "pop_den", "tree", "grassland", "shrub", "cropland", "built", "bare", "water", "wetland","mangrove","moss", "human_fp", "cattle","tsetse")], use = "pairwise.complete.obs")
...
p <- ggcorrplot(correlation_matrix, 
           ...
           lab = TRUE, 
           lab_size = 2.5,
           title = "Nigeria correlation matrix heatmap",
...
print(p)
ggsave("Code/Correlations/Correlation_heatmap_NGA.pdf", plot = p, 
